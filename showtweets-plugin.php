<?php
/**
 * Plugin Name: ShowTweets Plugin
 
 * Description: The showtweets plugin allows you to display your most recent tweets on your blog.
...
class showtweets_widget extends WP_Widget {

	function showtweets_widget() {
		...
		$widget_ops = array(
			'classname' => 'showtweets_widget',
			'description' => __('List your last tweet by displaying content, date, and link to follow you', 'tw_wp')
		);

		$control_ops = array();

		$this->WP_Widget('showtweets_widget', __('Twitter WP', 'tw_wp'), $widget_ops, $control_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("showtweets_widget");'));
 ...
 */
 if(!is_admin()) {
	...
	function twitter_wp_head() {
		...
		$array_widgetOptions = get_option('widget_showtweets_widget');
		...
		if ( $use_default_css )
			...
			$twitter_wp_css .= '<link type="text/css" media="all" rel="stylesheet" id="showtweets_widget_styles" href="'. plugins_url(MY_PLUGINBASENAME."/css/twitter_wp.css") . '" />';
